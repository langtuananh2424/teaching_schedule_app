  ...
  Future<void> _fetchMakeups() async {
    ...
    try {
      ...
      if (userRole == 'ROLE_MANAGER') {
        try {
          print('🔍 [MANAGER FILTER] Starting makeup session filtering...');
          ...
          if (email == null) {
            ...
            if (managerData.isEmpty) {
              ...
              if (managerDepartment == null || managerDepartment.toString().isEmpty) {
                ...
                // Debug: Kiểm tra lecturerName trong makeup sessions
                print('📝 Checking lecturerNames in makeup sessions:');
                for (var i = 0; i < makeups.length && i < 5; i++) {
                  print('   - Makeup ${makeups[i].id}: name=${makeups[i].lecturerName}');
                }

                // ✅ Lọc makeup sessions theo tên lecturer (department-based)
                filteredMakeups = makeups.where((makeup) {
                  final matches = departmentLecturerNames.contains(makeup.lecturerName);
                  if (matches) {
                    print('   ✓ Makeup ${makeup.id} matches: ${makeup.lecturerName}');
                  }
                  return matches;
                }).toList();

                print(
                  '✅ Filtered to ${filteredMakeups.length} makeup sessions for department $managerDepartment',
                );
              }
            }
          }
        } catch (e, stackTrace) {
          print('❌ Error filtering makeup sessions by department: $e');
